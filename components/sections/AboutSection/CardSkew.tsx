...
export default function GithubCardSkew({ className }: { className?: string }) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "flex transform-gpu flex-col gap-4 rounded-3xl border border-border text-zinc-200 shadow-lg transition-transform ease-linear will-change-transform",
        className,
      )}
      style={{
    ...
    >
      <img
        className="w-[260px] rounded-xl lg:w-[230px]"
        src="/profile.webp"
      ></img>
    </div>
  );
